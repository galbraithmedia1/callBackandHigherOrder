...
function uniq (array, callback){
  ...
  for (let i = 0; i < array.length; i++)
      if(!safe.includes(array[i])){
        safe.push(array[i])
      }

      // const results = array.filter(element => {
      //   return safe.includes(element)
      // });
...
uniq(names, (uniqArr)=>{
  console.log`the new names arry with all the dup items removed is ${uniqArr}`
})
...
const each = (array,callback)=>{
  array.forEach(name => {
    callback(name, array.indexOf(name))
  });
}
