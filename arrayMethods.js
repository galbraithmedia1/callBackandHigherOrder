...
const postTaxPrices = prices.map((price) => {
  return Math.round(price * 1.07 * 100) / 100;
});
...
// CODE HERE
const myStrongest = monstersInYourPocket.filter(monsterObject => {
  return monsterObject.CP > 200;
});
...
const applyTaxRate = orders.map(order =>{
  return  Math.round((order.price + (order.price * order.tax)) * 100)/100
}
  )

console.log(applyTaxRate)
////////// PROBLEM 6 //////////
...
// CODE HERE
const bobsTotal = purchases.filter(purchase => purchase.owner === 'Bob').reduce((acc, curr) => {
